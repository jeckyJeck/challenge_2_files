...
def cool_cast(img):
    return color_cast(img, (0.75, 0.95, 1.3))
...
def solarize(img):
    return ImageOps.solarize(img.convert("RGB"), threshold=110)
...
def gaussian_blur(img):
    return img.convert("RGB").filter(ImageFilter.GaussianBlur(2.2))
